...
function renderProjectRow(p) {
  ...
  return `<article class="project-card"><div class="project-main"><button class="project-title" data-open="${esc(p.id)}">${esc(p.name)}</button><div class="project-meta"><span>${esc(t?.name || p.typeName || 'No type')}</span><span>${p.billable ? `${money(p.rate)}/hr` : 'Not billable'}</span><span>${durLong(projectSeconds(p.id))}</span><span class="charge-readout">${money(charge(p.id))}</span>${p.useCap ? '<span class="cap-chip">10h cap</span>' : '<span class="cap-chip">No cap</span>'}</div></div><div class="project-actions">${r.length ? `<span class="timer-readout">${dur(elapsed(r[0]))}</span><button class="button button-danger" data-toggle="${esc(p.id)}">Stop</button>` : `<button class="button button-primary" data-toggle="${esc(p.id)}">Start</button>`}${r.length > 1 ? `<span class="status-chip">+${r.length - 1} more</span>` : ''}<button class="button" data-open="${esc(p.id)}">Open</button></div></article>`;
}
...
function renderProject(projectId) {
  const p = projectById(projectId);
  if (!p) return `<section class="panel"><h1>Project not found</h1><p class="muted">Wrong profile, deleted, or a small database troll.</p><button class="button" data-nav="timers">Back</button></section>`;
  const r = running().filter(s => s.projectId === p.id);
...
function bindBillingControls() {
  ...
  ROOT.querySelector('[data-bill-select-all]')?.addEventListener('change', e => {
    ROOT.querySelectorAll('[data-bill-select]').forEach(cb => cb.checked = e.target.checked);
    updateSelected();
  });
...
setDomTheme();
render();
setInterval(() => { if (running().length || stopwatch.running || countdown.running) render(); }, 1000);
